...
const DeleteModel = ({ application, onClose, onDelete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center backdrop-blur-sm z-50">
      <div className="bg-white rounded-xl shadow-lg p-6 w-11/12 md:w-1/3">
        ...
        <p>Are you sure you want to delete <strong>{application.applicant}</strong>'s application?</p>
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="bg-gray-300 px-4 py-2 rounded-lg hover:bg-gray-400">Cancel</button>
          <button onClick={() => { onDelete(application.id); onClose(); }} className="bg-red-500 px-4 py-2 rounded-lg text-white hover:bg-red-600">Delete</button>
        </div>
      </div>
    </div>
  );
