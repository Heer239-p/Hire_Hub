...
//   // Stats for cards - updated with professional colors
//   const stats = [
//     { 
//       label: "Total Jobs", 
//       value: jobCount, 
...
//       {/* Stats Cards - updated grid to always show 4 cards in a row */}
//       <section className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-10">
//         {stats.map(({ label, value, icon: Icon, bgColor, iconBg, textColor, path, trend, trendUp }, idx) => (
//           <div
//             key={idx}
//             onClick={() => navigate(path)}
//             className={`group relative overflow-hidden ${bgColor} hover:bg-opacity-80 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-2xl shadow-lg dark:shadow-gray-900/50 hover:shadow-2xl dark:hover:shadow-gray-900 transition-all duration-300 cursor-pointer border border-gray-200 dark:border-gray-700 hover:border-opacity-50 dark:hover:border-gray-500 hover:scale-[1.02]`}
//           >
//             {/* Content */}
//             <div className="relative p-6">
//               <div className="flex items-start justify-between mb-4">
//                 <div className={`p-3 rounded-xl ${iconBg} group-hover:bg-opacity-80 dark:bg-gray-700 dark:group-hover:bg-gray-600 transition-colors duration-300`}>
//                   <Icon className={`text-2xl ${textColor} group-hover:text-opacity-90 dark:text-gray-300 dark:group-hover:text-gray-200 transition-colors duration-300`} />
//                 </div>
//                 <div className={`flex items-center gap-1 text-sm font-semibold ${trendUp ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'} group-hover:text-opacity-90 dark:group-hover:text-opacity-90 transition-colors duration-300`}>
//                   <span>{trend}</span>
...
//               <div>
//                 <p className={`text-3xl font-bold ${textColor} group-hover:text-opacity-90 dark:text-gray-100 dark:group-hover:text-gray-200 transition-colors duration-300 mb-2 truncate`}>
//                   {value}
//                 </p>
//                 <p className="text-sm font-medium text-gray-600 dark:text-gray-400 group-hover:text-opacity-90 dark:group-hover:text-gray-300 transition-colors duration-300 uppercase tracking-wide">
//                   {label}
//                 </p>
//               </div>
//             </div>
...
//             {/* Decorative Element */}
//             <div className={`absolute -right-4 -bottom-4 w-24 h-24 rounded-full transition-all duration-500 ${iconBg} bg-opacity-20 group-hover:scale-110`} />
//           </div>
//         ))}
//       </section>
...
import React, { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { FiBriefcase, FiUsers, FiFileText, FiEye, FiDollarSign } from "react-icons/fi";
import {
  ResponsiveContainer,
  BarChart,
  Bar,
...
import { fetchAllPayments } from "../services/paymentService"; // Import payment service
...
const Dashboard = () => {
  ...
  const [userCount, setUserCount] = useState(0);
  const [applicationCount, setApplicationCount] = useState(0);
  const [totalIncome, setTotalIncome] = useState(0); // Add state for total income
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Fetch and calculate total income from payments
        const payments = await fetchAllPayments();
        const total = payments
          .filter(payment => payment.status === "Success") // Only count successful payments
